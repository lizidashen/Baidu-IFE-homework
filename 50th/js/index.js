...
CreateFormIndex.prototype = {
	...
	createForm:function(){//循环数组 根据数组的 title time state 生成样式.
		...
		for(let i = 0,length = bigarrbox.length; i<length; i++) {
			var div = $('<div>')
						.addClass('all_form_area_div row')
			...
			var	title = $('<span>')
						...
						.addClass('all_form_area_title col-xs-4')
						.appendTo(div);

			var time = $("<span>")
						.html(bigarrbox[i][0].time)
						.addClass('all_form_area_time col-xs-2')
						.appendTo(div);
			...
			switch(bigarrbox[i][0].state) {
				case 'on':
					var state = $("<span>")
								.html('发布中')
								.addClass("all_from_area_state col-xs-2")
								.appendTo(div);
					var buttonB = $('<button>')
								.html("删除")
								.addClass('all_from_area_button_delete col-xs-1')
								.appendTo(div);
					var buttonD = $('<button>')
								.html("填写")
								.addClass('all_from_area_button_look_from col-xs-1')
								.appendTo(div);
					var buttonC = $("<button>")
								.html('查看')
								.addClass('all_from_area_button_look_date col-xs-1')
								.appendTo(div);
						break;
	...
	buttonMouseLeave: function(e) {
		$(e.target).css('background-color','#ee7419');
	},
		...
		$('.all_from_area_button_delete, .all_from_area_button_look_from, .all_from_area_button_look_date')
			.on('mouseleave', function(e) {
				$(e.target)
					...
					.css("border-bottom", "1px solid #ee7419");
			})
